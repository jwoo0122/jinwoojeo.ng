...
ESM 은 ECMAScript 에 정의된 표준 Module 시스템입니다.[^1] CJS 에 비해 여러 장점들이 있는데요. Module 의 정의 및 가져오기는 아래와 같습니다.
...
아무튼, 글 초반에 활용했던 예제와 같습니다. ESM 에서는 `import` 와 `export` 라는 특수한 표현식(expression)을 이용해 모듈의 출력과 요청을 표현합니다. 이들 expression 에는 매우 중요한 규약이 있는데, 바로 **조건부 활용이 불가능하다는 점**입니다.[^2]
...
아쉽게도 ESM Loader는 이렇게 Monkey patching을 할 수는 없습니다. 대신 Loader Hook이라는 새로운 인터페이스를 통해 ESM Loader를 조작하는 방법을 제공하고 있어요.[^4] 자세한 내용은 [여기 공식 문서](https://nodejs.org/api/module.html#customization-hooks)를 참고해보세요.[^5]
...
[^1]: ESM module은 모두 strict mode로 취급됩니다. strict mode에 대해 궁금하시다면 [이쪽](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Strict_mode)을 읽어보세요.

[^2]: ESM에서도 물론 조건부 모듈 요청이 가능하긴 합니다. 이때는 `import` 표현식이 아닌 [`import()`](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Operators/import) 함수를 활용해야 합니다.

[^3]: 실제로 esbuild 의 결과물이 이렇지는 않으며, 이해를 위해 단순화한 점을 참고해주세요.

[^4]: Node.js는 preload script를 지정할 수 있는 옵션을 3개 갖고 있습니다. CJS script를 위한 [`--require`](https://nodejs.org/docs/latest/api/cli.html#-r---require-module) 옵션은 Node.js 1.6부터 있었고, Node.js 8.8.0에 ESM script를 위한 [`--(experimental-)loader`](https://nodejs.org/docs/latest/api/cli.html#--experimental-loadermodule) 옵션이 추가됩니다. `--loader`라는 이름에서 알 수 있듯 단순히 preload만을 하는 것이 아니라 Customization hook을 거는데 특화된 스크립트를 지정할 수 있는 옵션이었습니다. `--require`와 달리 preload, Hook registration 2개의 책임을 갖고 있었던 것이죠. 그래서 Node.js 20.6.0에서 이 옵션은 [`--import`](https://nodejs.org/docs/latest/api/cli.html#--importmodule)로 대체되었습니다. `--import`는 `--require`처럼 preload 만을 하며, 그저 CJS script가 아닌 ESM script를 받는다는 차이점만 있습니다. 훨씬 간단해졌죠!

[^5]: `--import` 혹은 `--loader` 옵션이 사용될 경우 entrypoint script가 항상 [ESM Loader를 통해 해석된다](https://github.com/nodejs/node/blob/0fb512344fe97b508adf1332c3e82d33937c4669/lib/internal/modules/run_main.js#L52-L87)는 점을 기억하세요.
